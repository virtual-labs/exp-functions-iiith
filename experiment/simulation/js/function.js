window.view = {
  ...
  displayFunctionForRectangle: function () {
    document.getElementById("functionRectangle").innerHTML +=
      "float area_rect (float a,float b)<br>{<br> &emsp; float area = a*b;<br> &emsp; return area;<br>}";
  },
  ...
  replaceDivs: function () {
    const instructionsOutput = document.querySelector(".instructionsOutput");
    if (this.i === 4) {
  ...
  area_sq: function (param, functionCall, beforeFunctionCall) {
    if (beforeFunctionCall !== 1) {
      functionCall = beforeFunctionCall + "*" + functionCall;
    }
    this.area = param * param * beforeFunctionCall;
    this.totalArea += this.area;
    alert(functionCall + " is " + this.area);
    this.correctInputs++;
  },
  area_triangle: function (param, functionCall, beforeFunctionCall) {
    if (beforeFunctionCall !== 1) {
      functionCall = beforeFunctionCall + "*" + functionCall;
    }
    this.area = (Math.sqrt(3) / 4) * param * param * beforeFunctionCall;
    this.totalArea += this.area;
    alert(functionCall + " is " + this.area);
    this.correctInputs++;
  },
  area_circle: function (param, functionCall, beforeFunctionCall) {
    if (beforeFunctionCall !== 1) {
      functionCall = beforeFunctionCall + "*" + functionCall;
    }
    this.area = Math.PI * param * param * beforeFunctionCall;
    this.totalArea += this.area;
    alert(functionCall + " is " + this.area);
    this.correctInputs++;
  },
  area_rect: function (param, functionCall, beforeFunctionCall) {
    if (beforeFunctionCall !== 1) {
      functionCall = beforeFunctionCall + "*" + functionCall;
    }
    this.area = Number(param[0]) * Number(param[1]) * beforeFunctionCall;
    this.totalArea += this.area;
    alert(functionCall + " is " + this.area);
    this.correctInputs++;
  },
  ...
  calculateArea: function () {
    ...
    var lineCount = 0; // Initialize line count for error reporting
    for (i = 0; i < elements.length; i++) {
      if (elements[i].tagName !== "INPUT") continue; // Skip non-input nodes
      lineCount++; // Increment line count only for input elements
      var parsedValue = elements[i].value.replace(/\s/g, ""); // Remove whitespace from input value
      var beforeFunctionCall = 1; // Default multiplier

      // Check if parsedValue starts with "<number>*"
      var multiplierMatch = parsedValue.match(/^(\d+(\.\d+)?)\*/);
      if (multiplierMatch) {
        beforeFunctionCall = parseFloat(multiplierMatch[1]); // Extract and parse the multiplier
        parsedValue = parsedValue.substring(multiplierMatch[0].length); // Remove the multiplier part from parsedValue
      }
      ...
      if (parameter.length === 1) {
        ...
        if (functionCall === "area_sq()" && isNaN(parameter) === false)
          this.area_sq(parameter, parsedValue, beforeFunctionCall);
        else if (
          functionCall === "area_triangle()" &&
          isNaN(parameter) === false
        )
          this.area_triangle(parameter, parsedValue, beforeFunctionCall);
        else if (functionCall === "area_circle()" && isNaN(parameter) === false)
          this.area_circle(parameter, parsedValue, beforeFunctionCall);
        else alert("Incorrect function call at line " + lineCount);
      } else if (
        parameter.length === 2 &&
        ...
        this.area_rect(parameter, parsedValue, beforeFunctionCall);
      else alert("Incorrect function call at line " + lineCount);
    }
